...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(self, meter_reading: MeterReading,
        ...
        pipeline.xadd(global_key, fields=MeterReadingSchema().dump(meter_reading), maxlen=self.GLOBAL_MAX_FEED_LENGTH)
        pipeline.xadd(site_key, fields=MeterReadingSchema().dump(meter_reading), maxlen=self.SITE_MAX_FEED_LENGTH)
        pipeline.execute()
